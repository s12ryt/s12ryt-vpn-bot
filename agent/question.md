...
- 部署後每 1 小時檢查目前已設定 REALITY 目標一次，沿用 DNS、TCP 443、TLS 1.3、憑證名稱與延遲驗證；未設定核心時安全略過，不掃描其他目標。
- 健康狀態須跨程序重啟持久化。通知採狀態轉換節流：首次明確故障通知所有 active 擁有者與管理員一次，持續故障不重複；恢復時再通知一次。檢查不得自動更換目標，仍由擁有者在 Web 搜尋並確認套用。
