...
- [x] 強化部署後 evidence 實體檢查：canonical containment、一般檔與 symlink 防護後再以 `-s` 拒絕 0-byte 空檔，避免只建立空白檔案就讓八類外部驗收證據通過；GitHub CI run `32078025866` 已驗證 race、vet、ShellCheck、PostgreSQL 17 integration、Web、Compose 與全部部署映像。
