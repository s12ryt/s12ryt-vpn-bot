...
- [x] 修復部署後檢查的完成語意：結構／API 檢查後必須讀取不含秘密的外部 E2E evidence JSON；四協定雙棧、IPv6-only／IPv4 出站、流量、配額、週期恢復、重啟與 600 連線任一缺少 `passed: true` 或非空 evidence 引用即 fail closed，不再以成功 exit code 搭配「需人工續驗」誤報完整驗收。
