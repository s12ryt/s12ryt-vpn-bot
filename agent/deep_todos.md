...
- [x] 補齊安裝預檢：Enter 正確採用公開位址候選、`-` 明確停用，重跑驗證並確認既有 `.env`；啟動前以 `getMe` 驗 Bot 身分、收集並驗證 ACME 非秘密參考值／候選 CA 條款及 Web HTTPS 拓撲。Bot token curl config 置於 0700 暫存目錄並以 signal／exit trap 清除；bootstrap URL／image 採白名單。自有網域依正式 schema 限 HTTP-01，禁止無法持久化 provider 憑證的 custom DNS-01。
