...
- [x] 補齊正式 release 的備份映像供應鏈：`s12ryt-vpn-backup` 先在 amd64 本地建置並經嚴格 Trivy HIGH／CRITICAL 閘門，通過後才發布 amd64／arm64、SBOM、provenance；release metadata 記錄 backup digest，首次套件可見度提醒與 README 同步涵蓋備份套件。
