...
- [x] 修復安裝精靈公開位址契約：IPv4／IPv6 各向 api.ipify.org、ifconfig.co、icanhazip.com 查詢，只有至少兩個來源一致才列為可信候選；部署者可修改或停用單一 address family，至少保留一個 family 並明確確認後才可進入 Compose 驗證與啟動。新建 `.env` 保存確認值供 Web 核心設定核對，並移除已失效的環境式 `BACKUP_RETENTION_DAYS`。
