...
外部客戶端與負載測試完成後，建立不含 token、密碼或憑證的 JSON manifest。每個 `evidence` 必須是相對於 manifest 所在目錄的安全路徑，指向實際存在、非 symlink 且非空的測試輸出、監控快照或變更紀錄；絕對路徑、`..`、空檔與經父層 symlink 逃出該目錄的引用都會被拒絕：
